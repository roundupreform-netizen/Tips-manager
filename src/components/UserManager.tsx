...
import { storage, getDefaultPermissions, getRandomAvatarColor } from '../lib/storage';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">Manage internal staff accounts and access rights</p>
          </div>
        </div>
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
            <div className="relative">
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-bold text-slate-700"
              />
            ...
            <div className="pt-4 border-t border-slate-50 grid grid-cols-2 gap-4">
              <div className="bg-emerald-50 p-4 rounded-2xl flex flex-col items-center">
                <span className="text-2xl font-black text-emerald-600">{users.filter(u => u.active).length}</span>
                <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest">Active</span>
              </div>
              <div className="bg-rose-50 p-4 rounded-2xl flex flex-col items-center">
                <span className="text-2xl font-black text-rose-600">{users.filter(u => !u.active).length}</span>
                <span className="text-[10px] font-black text-rose-800 uppercase tracking-widest">Disabled</span>
              </div>
            </div>
          </div>

          <div className="bg-slate-900 p-8 rounded-[2rem] text-white space-y-4 relative overflow-hidden shadow-xl">
            ...
            <ShieldCheck className="absolute -bottom-4 -right-4 text-slate-800 w-32 h-32 rotate-12" />
          </div>
        </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Staff Info</th>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-slate-900">{editingUser ? 'Edit Staff Account' : 'New Staff Account'}</h2>
                  <p className="text-sm font-medium text-slate-500">Define credentials and system role</p>
                </div>
              ...
              <form onSubmit={handleSave} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col items-center justify-center space-y-4 pb-4">
                    <div className="relative group">
                      ...
                      <button
                        ...
                        className="absolute bottom-0 right-0 w-8 h-8 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white hover:bg-indigo-500 transition-all scale-0 group-hover:scale-100 origin-bottom-right"
                      >
                        <Camera size={14} />
                      </button>
                      <input 
                    ...
                    <div className="flex gap-2">
                      {['#6366f1', '#ec4899', '#f59e0b', '#10b981', '#3b82f6'].map(color => (
                        <button
                          ...
                          className={cn(
                            ...
                            formData.avatar === color ? "border-slate-900 scale-110" : "border-transparent opacity-60 hover:opacity-100"
                          )}
                      ...
                      <button
                        ...
                        className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-all"
                      >
                        <Palette size={12} />
                      </button>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 font-bold"
                      placeholder="e.g. Rahul Sharma"
                    />
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Staff Username / Identifier</label>
                    <div className="relative">
                      <input 
                        required
                        value={formData.username}
                        onChange={e => setFormData({...formData, username: e.target.value})}
                        className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 font-bold"
                        placeholder="rahul_s"
                      />
                  ...
                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-sm text-slate-900">User Permissions</p>
                        <p className="text-[10px] text-slate-500 font-bold uppercase">Configure access rights</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsPermissionsOpen(true)}
                      className="px-4 py-2 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-500 transition-all"
                    >
                      Customize
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        ...
                        formData.active ? "bg-emerald-100 text-emerald-600" : "bg-slate-200 text-slate-400"
                      )}>
                        {formData.active ? <UserCheck size={20} /> : <UserX size={20} />}
                      </div>
                      <div>
                        <p className="font-bold text-sm text-slate-900">Account Status</p>
                        <p className="text-[10px] text-slate-500 font-bold uppercase">Currently {formData.active ? 'Active' : 'Disabled'}</p>
                      </div>
                    </div>
                    <button
                      type="button"
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    type="button"
                    onClick={resetForm}
                    className="flex-1 py-4 font-black text-slate-500 uppercase tracking-widest hover:bg-slate-50 rounded-2xl transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 active:scale-95 uppercase tracking-widest"
                  >
        ...
        {isPermissionsOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md">
            <motion.div 
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white w-full max-xl rounded-[2.5rem] shadow-2xl overflow-hidden"
            >
              <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-900 uppercase">User Rights Control</h2>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Assign granular permissions for {formData.name || 'Staff'}</p>
                  </div>
                </div>
                <button onClick={() => setIsPermissionsOpen(false)} className="p-2 hover:bg-slate-200 rounded-xl transition-colors">
                  <X size={24} />
                </button>
              </div>
                ...
                {Object.entries(formData.permissions).map(([key, value]) => (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      value ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-300 group-hover:bg-slate-200"
                    )}>
